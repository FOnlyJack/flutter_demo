...
Future getSearchResult(int _currentIndex,{formData}) async{
  try{
    Response response;
    Dio dio = new Dio();
    response = await dio.post(serviceUrl+"article/query/$_currentIndex/json",data:formData);
    if(response.statusCode==200){
      return response.data;
    }else{
      throw Exception('后端接口出现异常，请检测代码和服务器情况.........');
...
Future projectTabData(_currentIndex,cid) async{
  try {
    Response response;
    Dio dio = new Dio();
    response = await dio.get( serviceUrl+"project/list/$_currentIndex/json?cid=$cid");
    if (response.statusCode == 200) {
      return response.data;
    } else {
      throw Exception('后端接口出现异常，请检测代码和服务器情况.........');
